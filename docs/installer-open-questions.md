# Calciforge Installer — Open Questions & Scope Not Yet Implemented
...
- This is a `calciforge sync-policy` concern as much as install-time
...
- This is the "secure by default" story: Calciforge never uses root SSH

### 3. Service install/management
- Installing NZC or Calciforge binary on remote hosts
...
Session 4 needs similar structs in the Calciforge crate.
**Should these be in a shared `calciforge-common` crate?** Currently they'd be duplicated.
...
> "the name is calciforge so we need to assume many [claws]"
> "installer running on one machine while targeting a remote machine, in fact several remote machines"
> "NZC target, calciforge target and openclaw target need to be separately configurable endpoints (and separately configured SSH keys/user accounts)"
