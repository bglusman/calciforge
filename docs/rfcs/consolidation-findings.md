# Consolidation findings — #28 (secrets → security-proxy merge)
...
**Summary**: `security-proxy` and `secrets-client` have parallel credential
...
- `secrets-client/src/vault.rs` line 23:
  ```rust
...
that secrets sees it but security-proxy does not. If both services are
...
- `secrets-client`: matches by **URL path segment** (`/proxy/openai/v1/chat/completions` → provider = "openai"). Client must know the convention.
...
1. `/proxy/:provider/...` — explicit path-based (legacy secrets)
...
- `secrets-client`: only `Authorization: Bearer` (all providers) plus
  special-case `X-Subscription-Token` for `brave`.

**Implication**: if a request for anthropic goes through secrets-client,
it sets `Authorization: Bearer ...` which anthropic will reject (they
want `x-api-key`). The e2e tests at
`crates/calciforge/tests/e2e/secrets_proxy.rs` may have been working only
...
- `secrets-client` has no equivalent — derives env var name from the
...
## Finding 5: Resolver in secrets ≠ injector in security-proxy

- `secrets-client/src/vault.rs::get_secret()` is a full resolver: env →
...
security-proxy starts, security-proxy doesn't pick it up. secrets picks
...
## Finding 6: Hardcoded "vault.example.com" in `vault.rs`

`VaultConfig::default` sets `url` to `"https://vault.example.com"` when
`SECRETS_VAULT_URL` is unset. This is a **hardcoded fallback URL
...
"vaultwarden URL not configured; set SECRETS_VAULT_URL".
...
Before deletion in this branch, `secrets-client/src/policy.rs` was
...
security-proxy nor secrets-client implements this today.** All current
...
`crates/secrets-client/tests/vault_fallthrough.rs` — LLM-generated tests
...
## Finding 10: e2e secrets proxy test assumes a running service

`crates/calciforge/tests/e2e/secrets_proxy.rs` has:
```rust
async fn start_secrets() -> String {
    // ...
    // Probably returns localhost URL, tests skip if unreachable
}
```
And there's a skip message: "Skipping test: OneCLI not running on {}".
So these tests *opt out* when secrets isn't up. After the binary goes
...
- [ ] Delete `secrets-client/src/main.rs` (after security-proxy
      absorbs `/vault/:secret` and `/proxy/:provider`)
- [ ] Delete `security-proxy/src/credentials.rs` duplicate — replace
      with `secrets-client/src/vault.rs::get_secret` (finding 1, 5)
...
- [ ] Remove hardcoded `vault.example.com` fallback (finding 6)
- [ ] Stub substitution module returning "no references found" so #29
      can slot in without API churn (finding 8)
- [ ] Rename crate `secrets-client` → TBD (candidate:
      `secret-proxy-client`); update workspace Cargo.toml, CI matrix,
      calciforge's path dep
- [ ] Update e2e tests in calciforge crate to hit security-proxy
      instead of secrets (finding 9)
- [ ] Update install.sh: remove secrets bin, keep fnox install, add
