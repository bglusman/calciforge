...
> **Note:** Claude Code cannot run as root. If Calciforge runs as root, create a dedicated user:
> ```bash
...
Credentials are stored in `~/.claude/.credentials.json`. If Calciforge runs as a different user (e.g. root), copy the credentials:
...
Claude Code credentials aren't accessible to the user running Calciforge. Copy `~/.claude/.credentials.json` to the home dir of the running user.

**Raw `[client]`/`[tool]`/`[thinking]` output sent to users**
This is filtered by Calciforge's acpx adapter (`strip_acpx_noise()`). If you see it, you may be running an older build.

**Slow first response**
acpx creates a new session on first use. Subsequent messages in the same cwd reuse the session and are faster.

**`--format` flag errors**
The `--format` flag must come before the agent name: `acpx --format text claude exec "..."`. Calciforge handles this correctly as of commit 5f9d9bb.
