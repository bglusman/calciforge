...
PolyClaw supports routing messages to [Claude Code](https://code.claude.com) via the `acpx` CLI adapter. This lets you chat with Claude through WhatsApp, Signal, Telegram, or any other PolyClaw channel.
...
> **Note:** Claude Code cannot run as root. If PolyClaw runs as root, create a dedicated user:
> ```bash
...
Credentials are stored in `~/.claude/.credentials.json`. If PolyClaw runs as a different user (e.g. root), copy the credentials:
...
Claude Code credentials aren't accessible to the user running PolyClaw. Copy `~/.claude/.credentials.json` to the home dir of the running user.

**Raw `[client]`/`[tool]`/`[thinking]` output sent to users**
This is filtered by PolyClaw's acpx adapter (`strip_acpx_noise()`). If you see it, you may be running an older build.

**Slow first response**
acpx creates a new session on first use. Subsequent messages in the same cwd reuse the session and are faster.

**`--format` flag errors**
The `--format` flag must come before the agent name: `acpx --format text claude exec "..."`. PolyClaw handles this correctly as of commit 5f9d9bb.
