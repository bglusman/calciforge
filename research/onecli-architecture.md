...
_Sources: github.com/secrets/secrets README, secrets.sh/blog posts_
...
**This is the piece that could be embedded or run alongside NZC/Calciforge.**
...
```bash
# Setup example
secrets provider add bitwarden --vault-url "https://vault.bitwarden.com"
secrets rules create --name "Stripe rate limit" \
...
```bash
git clone https://github.com/secrets/secrets.git
cd secrets
...
### What this means for NZC/Calciforge integration:
...
3. **Dependency story for NZC**: NZC cannot link OneCLI's gateway as a Rust library dependency. The options are:
   ...
   - **Implement the same proxy pattern natively** in NZC/Calciforge (build a credential-injecting HTTP proxy from scratch in Rust)
   - **Use OneCLI as a managed service** (their cloud offering at app.secrets.sh, or self-hosted)
...
## Relevance to Calciforge's Design

### What Calciforge Could Borrow Conceptually
...
This pattern can be implemented natively in Calciforge/NZC without using OneCLI at all.

### What Calciforge Might Actually Use

For early implementation, running OneCLI as a sidecar alongside Calciforge is the **fastest path to get credential injection working**. The integration surface is small:
...
### What's Missing from OneCLI for Calciforge's Use Case
...
OneCLI's blog post explicitly says "approval workflows" (pause + ask human) are "coming next" for high-risk actions — but the implementation will be through their own UI, not via Signal/Telegram. **Calciforge's chat-based approval relay is genuinely novel and not served by OneCLI.**
...
**Phase 2**: Calciforge implements its own approval relay on top — when an agent requests a credential that requires human approval, Calciforge intercepts (either via OneCLI webhook/API or by implementing its own proxy) and sends an approval request to the user via Signal/Telegram. Once approved, credential is released.
...
This is directly applicable to Calciforge's multi-claw design.
