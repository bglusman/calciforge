...
The 2026-03-30 incident: Librarian tried to add a Calciforge channel adapter to `openclaw.json` manually, wrote invalid config, the gateway crashed, and Custodian couldn't recover it. Manual intervention required.
...
This is Workstream 4 from the vault integration plan. It's a research/design task for now; implementation goes into Session 3 (Calciforge adapter) and beyond.
...
- **vs Proposal B (Calciforge meta-commands):** Calciforge doesn't exist yet as a working adapter layer. We need safety now, for Session 3. B requires Calciforge to intercept tool calls or inject context — complex to build and test before the adapter is done. Defer B until Session 3 is shipped.
...
**This is the only thing that needs to be shipped for Session 3.** The Calciforge adapter installer needs to:
...
## What Should Go Into Session 3 (Calciforge Adapter)

Session 3 is the Calciforge adapter installation — editing live OpenClaw config. This is the highest-risk operation in the current sprint.
...
5. **The Calciforge adapter installer uses `transaction_start` explicitly** — don't rely on the auto-backup safety net for the most dangerous operation. Be explicit.

**Defer to after Session 3:**

- Proposal B (Calciforge meta-commands) — needs Calciforge adapter architecture first
...
The core constraint: **local transactions do not protect remote hosts.** When the Calciforge adapter installer SSHes to Librarian at 10.0.0.20 to edit `openclaw.json`, the transaction is running on the machine where NZC is running (the Calciforge host). If NZC is on a different host from Librarian, the transaction doesn't cover the edit.
...
Build Proposal A (explicit transaction API) + Level 0 (backup copy) for Session 3. Ship with `mode = "suggest"` auto-protection. Upgrade to Level 2 (native overlayfs via bwrap) after Session 3. Add Proposal B (Calciforge meta-commands) when the Calciforge adapter layer exists. Configure ZFS snapshot guards for known ZFS hosts. Accept that non-ZFS remote hosts are a persistent gap and document it clearly.
