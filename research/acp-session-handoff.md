...
For the Calciforge use case, a **Session Proxy with Snapshot Fallback** architecture is recommended:

1. **Primary flow**: Calciforge runs a lightweight session proxy that both desktop and mobile connect to
...
1. **Does Calciforge already implement any session proxy functionality?** The Big Hat Group blog mentions using `sessions_spawn` with `runtime: "acp"` for persistent sessions - investigate if this can be leveraged.
...
This section documents practical, client-specific mechanisms for session handoff that can be leveraged by Calciforge, regardless of ACP protocol limitations.
...
#### Key Findings for Calciforge Integration
...
**Workaround:** Use Claude Code's `--remote` flag to create web sessions, then access via Calciforge's browser capabilities if it has web view support.

---

### A.2 OpenCode: Native HTTP Server + Multi-Client Support

OpenCode has **excellent support for session sharing** through its HTTP server mode, which is **directly exploitable** by Calciforge.
...
# Mobile: Connect to server via Calciforge tunnel
...
|----------|-------------|----------------|
| **Direct HTTP proxy** | ✅ High | Proxy OpenCode's HTTP API through Calciforge |
| **Tailscale integration** | ✅ High | Use Tailscale for secure mobile→desktop routing |
| **SSE event streaming** | ✅ High | Calciforge can consume real-time event stream |
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────────┐
│   Mobile     │◄───►│   Calciforge   │◄───►│  OpenCode Server │
│  (Calciforge)  │     │   (proxy)    │     │  (desktop)       │
└──────────────┘     └──────────────┘     └──────────────────┘
                           │                         │
                           │    ┌────────────────────┘
                           │    │
...
| Client | Mechanism | Multi-Client | Bidirectional Handoff | Calciforge Integration |
|--------|-----------|--------------|----------------------|---------------------|
...
### A.5 Recommended Calciforge Architecture

Based on this research, the optimal approach for Calciforge:

#### Phase 1: OpenCode Integration (Immediate)

```yaml
# Calciforge configuration snippet
opencode:
  enabled: true
...
2. Calciforge discovers server via mDNS or explicit config
3. Calciforge proxies HTTP API to mobile interface
...
**Implementation:**
1. Calciforge monitors session state via ACP
...
2. Mobile: Access claude.ai/code via Calciforge browser
...
| Client | Handoff Support | Calciforge Integration |
|--------|----------------|---------------------|
...
2. Use HTTP API proxying through Calciforge for mobile access
...
1. **Prototype OpenCode integration:**
   ...
   # Test mobile access via Calciforge proxy
   ```
