...
_Note: Sections 7-8 from original draft removed — "transparent runtime fallback" framing was incorrect. See vault-integration-plan.md Workstream 3 for correct framing: Calciforge routes channels, reassignment is explicit with context handoff._
...
**What can be migrated**: channel credentials (bot tokens, account IDs, allowlists) for channels Calciforge/NZC will own.
...
| OpenClaw field | Calciforge/NZC equivalent |
|----------------|------------------------|
...
When Calciforge reassigns a channel from OpenClaw to NZC (or vice versa), the new owner needs context. Options:

1. **Recent transcript dump**: Calciforge reads last N turns from the old claw's session and passes them to the new claw on first message
...
These could be used by Calciforge to fetch context from an OpenClaw instance during reassignment.
