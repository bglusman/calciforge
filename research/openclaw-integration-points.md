# OpenClaw Channel/Plugin API — Integration Points for Calciforge
...
There is **no native "calciforge channel" type** in OpenClaw's current schema. The right integration point depends on what Calciforge needs to do:
...
| Add a new inbound channel | Plugin with `channels: ["calciforge"]` manifest | New/plugin-required |
...
```json5
{
  hooks: {
    enabled: true,
    token: "shared-secret-between-calciforge-and-openclaw",
    path: "/hooks",
...
**Calciforge → OpenClaw (inbound message)**:
```bash
...
Authorization: Bearer <hooks.token>
{
  ...
  "agentId": "main",
  "sessionKey": "calciforge:telegram:+15551234567",   # stable per-user session key
  "wakeMode": "now",
  "deliver": false,          # Calciforge will handle delivery
...
**What `deliver: true` does**: OpenClaw can deliver the response itself via whatever channel is configured (last used channel, or a specific `to:` target). This could work as a fallback delivery mechanism if Calciforge wants OpenClaw to own the reply.

**Limitation**: There's no persistent two-way channel. Calciforge sends a request, gets a response — it's fundamentally request/response, not a streaming bidirectional channel.
...
Authorization: Bearer <gateway.auth.token>
{
  ...
  "user": "calciforge:telegram:+15551234567",  # stable session key derivation
  "stream": false
}
```
...
**Streaming**: Set `"stream": true` → SSE stream. Calciforge can relay tokens to the user as they arrive.

**Session routing**: The `user` field creates a stable session key. If Calciforge consistently sends the same `user` value for the same sender, OpenClaw maintains a persistent session context for that person.
...
- The channel sends/receives to Calciforge's endpoint
- OpenClaw treats it like any other channel (Telegram, Signal, etc.)

**What this enables**:
- Calciforge gets proper `channels.calciforge` config block in `openclaw.json`
- OpenClaw's routing/allowlist/DM policy applies to Calciforge messages
- Session scoping, group policies, mention gating all work correctly
- Calciforge messages appear in OpenClaw's session history properly tagged

**What's required**:
- Write the plugin (TypeScript, registers a channel)
- Install it on the target OpenClaw instance
- Add `channels.calciforge` config block
...
`POST /tools/invoke` lets Calciforge call any allowed tool directly:
...
- Calciforge reading OpenClaw's memory to provide context
...
## Recommended Integration Strategy for Calciforge

### Phase 1: No Config Changes Required

Use the chat completions endpoint. Only requirement: `chatCompletions.enabled: true` in config (one field, minimal risk). Calciforge acts as a thin proxy.

```
Calciforge receives message → POST /v1/chat/completions → relay response back
```
...
Install the Calciforge plugin on the OpenClaw instance. This requires the full safe installer flow (backup → version check → schema validate → diff → confirm → health check → rollback path).

This is the cleanest long-term architecture but needs the most setup work and is the highest risk during install.

---

## What Calciforge Should NOT Do

Based on the 2026-03-30 incident:

1. **Don't add config keys that don't exist in the schema** — this crashes the gateway
2. **Don't add a `channels.calciforge` entry without installing the plugin first** — plugin must be installed before its channel ID is referenced
...
**Bundled hooks** (events Calciforge could subscribe to):
...
**Important**: `plugins.entries.<id>.hooks.allowPromptInjection` defaults to `false`. Core blocks `before_prompt_build` and prompt-mutating fields from `before_agent_start`. Calciforge plugin would not be able to inject into the system prompt without explicit config opt-in.
