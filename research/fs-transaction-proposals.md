...
_Context: Three concrete interface proposals for the NZC/Calciforge transaction system_
...
```
// Agent is about to edit openclaw.json for the Calciforge adapter installation

txn = transaction_start(
    label="calciforge adapter installation - openclaw.json edit",
    backend="auto",
...
if health_check_passes:
    transaction_commit(txn, message="Added calciforge channel adapter")
...
## Proposal B: Calciforge Meta-Command Interface

### The API

Calciforge exposes meta-commands that operators (human or automated) can invoke:
...
These are delivered as Calciforge-level messages, not agent tool calls. Calciforge intercepts them before routing to the underlying claw.

### How Calciforge Coordinates with the Active Claw

**Problem:** Calciforge knows `!txn start` was invoked, but how does it know what filesystem operations the claw subsequently performed?

**Options:**

**Option B1: Agent self-reports (lightweight)**
- `!txn start` instructs Calciforge to inject a system prompt addition into the agent's context: "You are now inside transaction TXN-123. Before modifying any file, call `transaction_file_declare(path)` to register it with the transaction."
- The agent declares files it intends to modify; Calciforge/NZC backs them up before the write.
- On `!txn rollback`: Calciforge sends a tool call to NZC to restore all declared files.

**Option B2: Calciforge intercepts tool calls (strong)**
- Calciforge wraps every outbound tool call through its routing layer.
- When `write(path, ...)` or `edit(path, ...)` passes through while a transaction is active, Calciforge triggers a pre-write backup.
- No agent cooperation needed.

Option B2 is architecturally better but requires Calciforge to understand NZC's tool call format, which couples them. For now, B1 is more realistic.
...
| OpenClaw | Calciforge cannot intercept OpenClaw's file writes | Only if agent declared files via B1 |
...
```
[Human to Calciforge]
!txn start "add Calciforge channel to Librarian"
...
[Human]
Now edit /root/.openclaw/openclaw.json to add the calciforge channel.

[Agent executes tool calls; Calciforge intercepts write(openclaw.json, ...)]
[Calciforge auto-backs up openclaw.json before write proceeds]

[Agent]
Done. Gateway restarted. Health check passed.

[Human]
!txn commit --message "Calciforge adapter added successfully"
...
**Human-initiated nesting is not recommended.** `!txn start` while another is active: Calciforge prompts "Transaction TXN-abc123 is already active. Nest inside it? [yes/no]"

Nesting semantics mirror Proposal A: inner rollback doesn't affect outer; outer rollback restores everything.

**Cross-SSH:** Not supported. `!txn start` protects the local claw's filesystem. When the agent SSHes away, Calciforge should emit: "⚠️ Agent is crossing SSH boundary to 10.0.0.40. Remote operations are NOT covered by the active transaction. Consider !txn ssh-guard 10.0.0.40 to take a remote ZFS snapshot first."

### Crash Mid-Transaction

Same SQLite durability as Proposal A, but persisted in Calciforge's database (which spans multiple claws). On Calciforge restart:
...
```toml
[calciforge.transactions]
enabled = true
intercept_writes = true   # Intercept tool calls automatically (requires Calciforge to understand NZC tool format)
inject_on_start = true    # Inject "you are in a transaction" into agent context
default_backend = "auto"

[calciforge.transactions.ssh_guard]
...
**Cons:**
- Requires human to initiate — no automation benefit unless Calciforge auto-starts on certain triggers
- Cross-claw support is trust-based for non-NZC claws
- Calciforge must understand NZC tool call format for B2 interception — tight coupling
...
| Dimension | Proposal A (Explicit NZC) | Proposal B (Calciforge Meta) | Proposal C (Implicit) |
|---|---|---|---|
...
3. Defer Proposal B (Calciforge meta-commands) until Calciforge's adapter layer is built (Session 3). The coordination mechanism depends on Calciforge architecture that doesn't exist yet.

This gives:
- Agents can be explicit and correct (Proposal A)
- Agents that forget get a safety net (Proposal C's suggest mode)
- No over-engineering before Calciforge is built
