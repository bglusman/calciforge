...
Add optional `input_device` hint to Calciforge channel config:
...
Calciforge passes this as envelope metadata to downstream claws. Claws can use it to:
...
This is free — Calciforge already routes per-channel. Just needs documentation and possibly a wizard step: "Is this channel primarily used from mobile?"
...
```toml
# ~/.calciforge/typo-profiles/brian.toml
[corrections]
"teh" = "the"
"hte" = "the"
"waht" = "what"
...
## Implementation in Calciforge vs NZC

**Calciforge:** Best place for layer 1 (channel metadata), layer 2 (convention documentation), layer 3 (platform detection), layer 4 (pre-pass annotator). Calciforge sees all messages before dispatch and has channel context.

**NZC:** Could implement layer 4 as a message pre-processor, layer 5 as a workspace file. But better to keep correction logic in Calciforge and have NZC just receive already-annotated messages.

---

## Open Questions for Brian

1. Should correction profiles be per-channel or per-user-identity? (User identity is more useful but Calciforge needs identity resolution first)
