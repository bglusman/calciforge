...
The new `json5.rs` module in `calciforge/src/install/` correctly implements the `escape_next` boolean pattern:
...
**NZC stub is appropriately limited:** `patch_nzc_config_stub` does a string-contains check for `[calciforge]` and appends if absent. It's idempotent and clearly marked as a stub.
...
`CommandHandler::new()` calls `load_active_agents()` which reads `~/.calciforge/state/active-agents.json`. This is a real file on disk, shared across all test invocations.
...
pub struct CommandHandler {
    // ... existing fields ...
    /// Directory for state persistence. Defaults to ~/.calciforge/state/.
...
|-----------|----------|-------------|
| calciforge | `/root/projects/calciforge` | Brian's monorepo. Contains `nonzeroclaw` and `calciforge` crates. Not a fork. |
...
**Branch:** `calciforge-patches` is the active branch. It has **9 commits** ahead of the merge base (commit `314e1d3`, the channel-approval-manager merge from upstream).

**Commit breakdown:**

| Commit | Author | Description | Upstreamable? |
|--------|--------|-------------|---------------|
| `a40218a` | Librarian | calciforge: initial patches (anthropic fixes, approval flow, openai_compat, clash crate) | Partly — anthropic fixes yes, calciforge-specific no |
...
**Summary:** 6 of 9 commits are upstream bug fixes that happened to land on this branch first. 3 are calciforge-specific infrastructure. **Upstream is 498 commits ahead** of `origin/main` (Brian's fork main isn't tracking latest upstream).

### Is calciforge's `nonzeroclaw` crate the same as the fork?

**No.** The `calciforge/crates/nonzeroclaw/` is a **separate codebase** — a reimplementation/fork-of-a-fork. It:
...
The `/root/projects/nonzeroclaw` fork is the zeroclaw codebase with patches. The `calciforge/nonzeroclaw` crate is a new, separate implementation that can optionally depend on the upstream `zeroclawlabs` crate for passthrough.
...
**Low for `calciforge/nonzeroclaw`:** Since it's a separate crate that optionally depends on `zeroclawlabs = "0.4"`, upstream releases don't break it. The risk is API surface drift: if upstream changes the types that NZC wraps in passthrough modules, those modules need updating.

**matrix-rust-sdk fork:** Unknown risk without examining the fork. Pinned by commit, so it's stable until someone bumps it.

### Recommendation

1. **Upstream the 6 bug-fix commits** from `calciforge-patches` to zeroclaw-labs via PRs. They're clean, authored by various contributors, and fix real bugs. This reduces fork maintenance burden and gives back to the project. Do this before the rebase becomes impossible.

2. **Rebase `calciforge-patches` onto latest `upstream/master`** after the PRs are merged. This will likely conflict on `Cargo.lock` and the `main.rs` namespace changes. Budget a half-day.

3. **For `calciforge/nonzeroclaw`:** keep it as-is. It's a separate crate, not a fork. The optional `zeroclawlabs` passthrough dep means it can track upstream releases at its own pace. When `zeroclawlabs` 0.5 ships, bump the dep and update passthrough modules.
...
**File:** `crates/calciforge/src/install/json5.rs` — `strip_json_comments`
...
**File:** `crates/calciforge/src/install/ssh.rs` — `shell_quote`
...
**File:** `crates/calciforge/src/auth.rs` — `resolve_channel_sender`
...
**File:** `crates/calciforge/src/install/executor.rs` — `patch_openclaw_config`
...
**File:** `crates/calciforge/src/commands.rs` — `active_agent_for`
...
**File:** `crates/calciforge/src/install/model.rs` — `backup_filename`
...
**File:** `crates/calciforge/src/install/model.rs` — `check_version_compatibility`
...
### From `crates/calciforge/src/install/`
...
**P0 — blocks real use:**
1. **NZC TOML patching** (`executor.rs:697`): The installer can't actually configure NZC targets. Anyone trying to install Calciforge with NZC agents hits a stub that appends raw text to TOML. Use `toml_edit` crate for proper in-place TOML modification.
...
2. **Fix the test failure** — `rm ~/.calciforge/state/active-agents.json` immediately, then add the `make_handler()` cleanup (Option C). 5-minute fix. Follow up with Option A (state_dir injection) when refactoring commands.rs.

3. **NZC TOML patching** — the stub is the biggest functional gap. Add `toml_edit` dep, implement proper `[calciforge]` section insertion/update. Half-day.
