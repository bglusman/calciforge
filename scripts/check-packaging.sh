...
ruby -e 'require "yaml"; ARGV.each { |path| YAML.safe_load(File.read(path), aliases: false, permitted_classes: []) }' \
...
if 'die "calciforge doctor reported errors while strict egress-proxy enforcement was enabled' not in body:
