//! `zeroclawed-mcp` — agent-facing secret-discovery MCP server.
//!
...
//!    have access to the full zeroclawed daemon (e.g., when running
//!    `claude` in a sandbox).
...
#[derive(Clone)]
pub struct ZeroclawedMcp {
    fnox: onecli_client::FnoxClient,
    tool_router: ToolRouter<Self>,
}

impl Default for ZeroclawedMcp {
    fn default() -> Self {
        Self::new(onecli_client::FnoxClient::new())
    }
}

impl ZeroclawedMcp {
    /// Construct a server bound to a specific [`onecli_client::FnoxClient`].
    /// Production callers use [`ZeroclawedMcp::default`] which uses
    /// `fnox` from `PATH`; tests inject a `FnoxClient::with_binary(path)`
    /// pointing at a fake script.
    pub fn new(fnox: onecli_client::FnoxClient) -> Self {
        Self {
            fnox,
...
impl ZeroclawedMcp {
    ...
    async fn list_secrets(&self) -> Result<CallToolResult, McpError> {
        match self.fnox.list().await {
            ...
            Err(onecli_client::FnoxError::NotInstalled(e)) => Err(McpError::internal_error(
                format!(
...
impl ServerHandler for ZeroclawedMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            protocol_version: ProtocolVersion::V_2024_11_05,
            capabilities: ServerCapabilities::builder().enable_tools().build(),
            server_info: Implementation {
                name: "zeroclawed-mcp".to_string(),
            ...
            instructions: Some(
                "ZeroClawed secret-discovery MCP. \
...
OUT"#,
        ...
        let server = ZeroclawedMcp::new(onecli_client::FnoxClient::with_binary(bin));
    ...
    async fn list_secrets_returns_actionable_error_when_fnox_missing() {
        let server = ZeroclawedMcp::new(onecli_client::FnoxClient::with_binary(
    ...
    fn server_advertises_tools_capability() {
        ...
        assert_eq!(info.server_info.name, "zeroclawed-mcp");
