...
impl WhatsAppChannel {
    ...
    pub async fn handle_message(
        ...
        // !secure — post-auth secret command. This must stay before the unknown-command
        // branch so secret material is never routed to an agent context.
        if CommandHandler::is_secure_command(&text) {
            ...
            tokio::spawn(async move {
                if let Err(e) = channel
                    ...
                    .await
                {
                    warn!(from = %from_owned, error = %e, "WhatsApp: failed to send secure-command reply");
                }
            });
            return;
        }
