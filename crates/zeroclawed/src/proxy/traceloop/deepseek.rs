...
impl Provider for DeepSeekProvider {
    ...
    async fn chat_completions(
        ...
        // DeepSeek API is OpenAI-compatible, so we can use similar logic
        ...
        // Note: DeepSeek may have different tool support than OpenAI
        ...
        if stream {
            ...
            Err(BackendError::NotAvailable(
                "Streaming not yet implemented for DeepSeek provider".to_string(),
            ))
        } else {
            // Parse the response
