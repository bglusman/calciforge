...
async fn env_var_wins_over_fnox() {
    ...
    let result = onecli_client::vault::get_secret("t4_envwins").await;
...
async fn env_lookup_uses_uppercase_api_key_suffix() {
    ...
    let result = onecli_client::vault::get_secret("mixed_case").await;
...
async fn fnox_value_returned_when_env_empty() {
    ...
    let result = onecli_client::vault::get_secret("VAULT_T4A_FNOX_WINS").await;
...
/// vaultwarden path (which will fail because ONECLI_VAULT_TOKEN is
...
async fn fnox_failure_falls_through_to_vault_error() {
    ...
    let result = onecli_client::vault::get_secret("VAULT_T4B_MISSING").await;
    ...
    assert!(
        err.contains("VAULT_T4B_MISSING")
            ...
            || err.contains("No ONECLI_VAULT_TOKEN"),
...
async fn fnox_binary_missing_is_graceful() {
    ...
    let result = onecli_client::vault::get_secret("VAULT_T4C_NO_FNOX").await;
...
async fn fnox_empty_output_is_rejected() {
    ...
    let result = onecli_client::vault::get_secret("VAULT_T4D_EMPTY").await;
