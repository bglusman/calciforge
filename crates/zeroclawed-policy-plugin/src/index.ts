...
export default definePluginEntry({
  ...
  register(api) {
    ...
    api.logger.info(`[zeroclawed-policy] clashd endpoint: ${config.clashdEndpoint}`);
    ...
    checkClashdHealth(config.clashdEndpoint).then((healthy) => {
      if (healthy) {
        ...
        api.logger.warn("[zeroclawed-policy] clashd health check: FAILED - policy enforcement may not work");
      }
    });

    // Register the before_tool_call hook
    api.registerHook("before_tool_call", async (context: HookContext): Promise<HookResult> => {
      const toolName = context.toolName;
      const args = context.args;
      const identity = context.session?.identity || "unknown";

      api.logger.debug(`[zeroclawed-policy] Evaluating: ${toolName} for ${identity}`);

      try {
        const verdict = await evaluateWithClashd(config, toolName, args, identity);

        if (verdict.verdict === "deny") {
          api.logger.info(`[zeroclawed-policy] DENIED: ${toolName} - ${verdict.reason}`);
          return {
            block: true,
        ...
        if (verdict.verdict === "review") {
          api.logger.info(`[zeroclawed-policy] REVIEW REQUIRED: ${toolName} - ${verdict.reason}`);
          return {
        ...
        api.logger.error(`[zeroclawed-policy] Error contacting clashd: ${errorMsg}`);
        
        // Fail-safe: configurable fallback
        if (config.fallbackOnError === "deny") {
          api.logger.warn(`[zeroclawed-policy] Falling back to DENY due to clashd error`);
          return {
            block: true,
            reason: `Policy enforcement unavailable: ${errorMsg}. Falling back to deny for safety.`,
          };
        } else {
          api.logger.warn(`[zeroclawed-policy] Falling back to ALLOW due to clashd error`);
          return { block: false };
        }
      }
    });
