...
async fn test_proxy_openai_models_endpoint() {
    ...
    let secrets_url = start_secrets().await;

    // When: We request the models endpoint through the proxy
    let client = reqwest::Client::new();
    let response = client
        .get(format!("{}/proxy/openai/v1/models", secrets_url))
    ...
    match response {
        ...
        Err(e) => {
            ...
            if e.is_connect() {
                println!("Skipping test: OneCLI not running on {}", secrets_url);
                return;
            }
...
async fn test_proxy_preserves_request_body() {
    ...
    let response = client
        .post(format!("{}/proxy/openai/v1/chat/completions", secrets_url))
...
async fn test_proxy_path_stripping() {
    ...
    for path in paths {
        let response = client.get(format!("{}{}", secrets_url, path)).send().await;
