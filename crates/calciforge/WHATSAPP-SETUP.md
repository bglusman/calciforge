...
# Forward incoming messages to Calciforge's webhook
...
Calciforge expects the standard WhatsApp Cloud API format:
...
The `from` field may omit the leading `+` — Calciforge normalises to E.164 format
...
```bash
# Check Calciforge webhook listener is up
