...
/// How Calciforge dispatches messages to a downstream claw, and whether the
...
/// A downstream claw that Calciforge will route messages to.
///
...
pub struct ClawTarget {
    /// Friendly name used in Calciforge config and logs (e.g. `"librarian"`).
    pub name: String,
    /// How Calciforge dispatches to this claw and whether remote config is possible.
...
/// Where Calciforge itself is running (may be local or remote).
#[derive(Debug, Clone)]
pub struct CalciforgeTarget {
    /// SSH target for Calciforge's host (`user@hostname`).
    pub host: String,
    /// SSH private key for Calciforge's host.
    pub ssh_key: Option<PathBuf>,
}
...
/// Top-level install descriptor: where Calciforge lives + all downstream claws.
#[derive(Debug, Clone)]
pub struct InstallTarget {
    /// Calciforge host metadata.
    pub calciforge: CalciforgeTarget,
