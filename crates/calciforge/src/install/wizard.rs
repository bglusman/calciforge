//! Interactive TUI wizard for the Calciforge installer.
//!
//! Launched when `calciforge install` is run with no CLI flags. Guides the
//! operator through configuring Calciforge to route messages to one or more
//! downstream claws.
//!
//! # Wizard steps
//!
//! 1. **Calciforge host** — where is Calciforge running? (host, SSH key)
...
//! 4. **Channel routing** — for each Calciforge-owned channel, which claw handles it?
...
const WIZARD_BANNER: &str = r#"
  ╔══════════════════════════════════════════════════════════╗
  ║         Calciforge Multi-Target Installer                  ║
  ║  Route messages from one Calciforge to many claws          ║
  ╚══════════════════════════════════════════════════════════╝
"#;
...
pub async fn run_wizard() -> Result<()> {
    ...
    if !<std::io::Stdin as IsTerminal>::is_terminal(&std::io::stdin()) {
        bail!(
            "interactive wizard requires a TTY — use --calciforge-host / --claw flags for non-interactive mode"
        );
    }

    println!("{}", style(WIZARD_BANNER).cyan().bold());

    // ── Step 1: Calciforge host ─────────────────────────────────────────────
    println!(
        "{}",
        style("Step 1/8 — Calciforge Host").bold().underlined()
    );
    let calciforge = collect_calciforge_host()?;
    ...
    let target = InstallTarget { calciforge, claws };
    ...
    let args = InstallArgs {
        calciforge_host: Some(target.calciforge.host.clone()),
        calciforge_key: target.calciforge.ssh_key.clone(),
        ..Default::default()
    };
    ...
    println!(
        "\n{}",
        style("All claws installed successfully! Calciforge is ready.")
            .green()
            .bold()
    );
    Ok(())
...
fn collect_calciforge_host() -> Result<CalciforgeTarget> {
    println!("  Where is Calciforge running?");
    println!("  (Enter 'local' if Calciforge runs on this machine)");

    let host: String = Input::new()
        .with_prompt("  Calciforge host (user@hostname or 'local')")
        .interact_text()
        .context("failed to read Calciforge host")?;
    ...
    Ok(CalciforgeTarget { host, ssh_key })
}
...
fn collect_channel_routing(target: &InstallTarget) -> Result<Vec<ChannelRouting>> {
    println!("  For each Calciforge-owned channel, choose which claw handles it.");
...
fn display_install_review(target: &InstallTarget) {
    println!(
        "  Calciforge host: {}",
        style(&target.calciforge.host).bold()
    );
