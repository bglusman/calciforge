...
//! keyed on the Calciforge sender identity so each user gets a persistent
...
impl HermesAdapter {
    ...
    async fn send_message(
        ...
        // Session continuity: use sender identity as session ID so each user
        // maintains a persistent conversation with the Hermes agent.
        if let Some(sender_id) = sender {
            req = req.header("X-Hermes-Session-Id", format!("calciforge-{sender_id}"));
        }

        info!(endpoint = %url, sender = ?sender, "hermes dispatch");
...
impl AgentAdapter for HermesAdapter {
    ...
    async fn dispatch_with_context(
        ...
        self.send_message(ctx.message, ctx.sender).await
    }
...
mod tests {
    ...
    async fn dispatch_with_session_header() {
        ...
        assert_eq!(*captured_session.lock().await, "calciforge-brian");
    }
}
