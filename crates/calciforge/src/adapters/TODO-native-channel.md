...
(format: `calciforge:{agent_id}:{sender}`). Requires on the OpenClaw side:
...
- Commit: `fix(calciforge): native channel adapters for OpenClaw and NZC with session continuity`
- Files changed:
