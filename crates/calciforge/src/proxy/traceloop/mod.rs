...
//! provider registry and routing system, adapted for calciforge.
...
impl TraceloopRouter {
    ...
    async fn get_cached_response(&self, cache_key: &str) -> Option<ChatCompletionResponse> {
    ...
    async fn select_fastest_provider(&self) -> Option<String> {
        ...
        let mut providers_with_latency: Vec<(String, u64)> = latency_stats
            ...
            .filter_map(|(provider_id, stats)| {
                stats
                    .average_latency_ms()
                    .map(|latency| (provider_id.clone(), latency))
            })
            .collect();
    ...
    pub async fn chat_completion(
        ...
        if stream {
            return self
                .chat_completion_without_cache(model, messages, stream, tools, tool_choice)
                .await;
        }
    ...
    async fn chat_completion_without_cache(
        ...
        let provider = self.registry.get(&provider_id).ok_or_else(|| {
            BackendError::NotAvailable(format!("Provider '{}' not found in registry", provider_id))
        })?;

        // Make the request
        let result = provider
            .chat_completions(model_name, messages, stream, tools, tool_choice)
            .await;
    ...
    async fn update_stats(&self, provider_id: &str, _in_progress: bool) {
        ...
        let provider_stats = stats
            .entry(provider_id.to_string())
            .or_insert_with(RouterStats::default);
